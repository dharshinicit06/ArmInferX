...
keeps startup fast and — critically — never loads the FP16 Transformers
baseline, which is infeasible on this machine's 7.63 GiB RAM.

Default engine: ``ARMINFERX_DEFAULT_ENGINE`` (default ``llamacpp-optimized``,
the verified Q4_K_M llama.cpp runtime on this machine).
"""
...
#: Q4_K_M llama.cpp engine on this machine.
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description=(
            ...
            "Serves text generation through a selectable inference engine "
            "(llama.cpp Q4_K_M) with per-engine benchmark metadata. Interactive "
            "docs for every endpoint are available here in Swagger UI."
        ),
        version="0.5.0",
        lifespan=lifespan,
        openapi_tags=[
